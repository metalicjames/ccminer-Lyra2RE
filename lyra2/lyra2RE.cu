...
//	for (int i = 0; i<4; i++)	{ printf("cpu keccak hash %d %08x %08x\n", i, hashB[2 * i], hashB[2 * i + 1]); }

	LYRA2(hashA, 32, hashB, 32, hashB, 32, 1, 8, 8);
//	for (int i = 0; i<4; i++)	{ printf("cpu lyra hash %d %08x %08x\n", i, hashA[2 * i], hashA[2 * i + 1]); }
...
//for (int i=0;i<4;i++)	{printf("cpu groestl hash %d %08x %08x\n",i,hash[2*i],hash[2*i+1]);}
    ...
    unsigned long *hashes_done)
{
	...
	const int throughput = 256*256*4;
