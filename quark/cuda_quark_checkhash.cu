...
__global__ void quark_check_gpu_hash_64_2(int threads, uint32_t startNounce, uint32_t *g_nonceVector, uint64_t *g_hash, uint32_t *resNounce)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
...
#pragma unroll 8
		for (int i = 0; i < 4; i++)
			LOHI(hash[2*i],hash[2*i+1],g_hash[i*threads+thread]);



		if (hash[7]<pTarget[7]) {resNounce[0] = nounce;}
	}
}
...
__host__ uint32_t quark_check_cpu_hash_64_2(int thr_id, int threads, uint32_t startNounce, uint32_t *d_nonceVector, uint64_t *d_inputHash, int order)
{
	uint32_t result = 0xffffffff;
	cudaMemset(d_resNounce[thr_id], 0xff, sizeof(uint32_t));
	...
	quark_check_gpu_hash_64_2 << <grid, block, shared_size >> >(threads, startNounce, d_nonceVector, d_inputHash, d_resNounce[thr_id]);
